...
- [ ] Add RED tests for fresh-ETag forwarding, missing/blank ETag no-write, HTTP 412 no-success/no-retry/no-verification, and successful conditional-write verification.
- [ ] Run `.venv/bin/python -m unittest tests.test_youtube_service tests.test_youtube_localization_api -v` and confirm only the new expectations fail.
- [ ] Implement the conditional call using the fetched resource's exact ID and ETag; convert 412/precondition failures to `YoutubeResetError` with actionable no-write wording; do not add a fallback without ETag.
...
- [ ] Add RED tests proving cards contain no Reset, no selected video hides Danger zone, a selected video scopes the reset target, and a stale pending ID produces a safe error without a service call; retain the `window.confirm` contract test.
- [ ] Run `.venv/bin/python -m unittest tests.test_sidebar tests.test_video_list -v` and confirm the new assertions fail.
- [ ] Remove per-card reset rendering; add selected-video Danger zone with exact title when available and always exact ID; retain browser confirmation and existing post-success invalidation.
- [ ] Add the pending/current selected-ID equality check immediately before service invocation, preserving URL and pagination state.
...
- [ ] Add RED tests for success/cache invalidation, unchanged no-change, stale/412 conflict without no-change text, and exception-only error output.
- [ ] Run the focused review/page tests and confirm the new expectations fail.
- [ ] Update the successful callback to invalidate only after `result.wrote is True`; change outcome branching so issues take precedence over no-change messaging.
- [ ] Re-run the focused tests and the full suite.
...
- [ ] Add RED tests for primary exclusion from options, empty/cleared references retaining primary, video-change reset, shared selection, visible three-step copy, inactive uploader, matching prompt activation, stale prompt rejection, and unchanged exact-code validation.
- [ ] Run the listed focused source/LLM tests and confirm the new assertions fail.
- [ ] Render the primary source as read-only, make the optional reference expander selectable only from non-primary existing localizations, and preserve all state normalization semantics.
- [ ] Render the External LLM instructions before checking prompt prerequisites; use a disabled uploader or explicit inactive placeholder with the required guidance while preserving current binding and validation.
- [ ] Re-run focused tests and the full suite.
...
- [ ] Add RED tests for missing OAuth file, malformed/wrong Desktop-app JSON, auth/401, callback port 8080, `quotaExceeded`, `YoutubeVideoNotFoundError`, generic network/API errors, severity, and secret redaction.
- [ ] Run the focused feedback/API/review tests and confirm the new cases fail.
- [ ] Consolidate existing `_render_service_error()` behavior into the shared helper and route bootstrap, selected-video loads, Preview, and Publish through it without broad exception-framework changes.
- [ ] Add only the minimal setup error context needed to distinguish missing and malformed OAuth files; preserve automatic refresh/re-auth and never delete `token.json` automatically.
- [ ] Re-run focused tests and the full suite.
...
- [ ] Add RED tests for non-logout status failure guidance and batch failure copy while asserting missing CLI and explicit logout messages remain unchanged and sanitized.
- [ ] Run the focused Codex tests and confirm the new expectations fail.
- [ ] Update only the user-facing failure strings and batch context propagation; do not change subprocess architecture or add recovery workflows.
- [ ] Re-run focused tests and the full suite.
...
- [ ] Update canonical English docs only after Phases 1–3 behavior is green: primary/reference flow, Publish refresh, Reset Danger zone/ETag/no-retry, External LLM three steps, setup recovery, and Codex failures.
- [ ] Update `AGENTS.md` so English canonical docs remain source of truth, localized ordinary-user guides live only under `docs/i18n/<locale>/README.md`, development docs stay English-only, and technical identifiers/UI labels remain literal.
- [ ] Add the compact 27-locale navigation line to the root README after the locale set is fixed.
- [ ] Check requested terms with `git grep` and run `git diff --check`.
...
- [ ] Add the same compact switcher to all 26 guides, linking to `../../../README.md` and sibling guides with correct relative paths; use native language names and no flags.
- [ ] Translate the same self-contained ordinary-user structure in batches of 4–6 locales: requirements, macOS/Linux and Windows PowerShell install, OAuth placement/authorization, Start, Translate, primary/reference source behavior, Codex, External LLM, Preview, Publish, Danger zone/Reset, troubleshooting, security, and License.
- [ ] Preserve commands, JSON, filenames, paths, environment names, URLs, BCP-47 codes, identifiers, product names, and literal UI labels exactly; explicitly protect `config/account_client_secrets_main.json`, `token.json`, and `token.pickle` in every guide.
- [ ] Verify every guide semantically matches the final English workflow and contains no invented capabilities or locale-specific behavior.
- [ ] Verify `find docs/i18n -mindepth 2 -maxdepth 2 -name README.md | wc -l` returns `26` and `docs/development.md`/`docs/superpowers/**` remain English/history-only.

### Task 9: Full verification and review

- [ ] Run all focused test commands after their phases and record results.
- [ ] Run `.venv/bin/python -m unittest discover -s tests -v` and confirm zero real YouTube/Codex calls.
- [ ] Run `.venv/bin/python -m compileall -q streamlit_app.py pages models.py language_catalog.py llm_localization_package.py codex_localization_runner.py codex_localization_generator.py generate_codex_localizations.py services state ui youtube_account.py localizations.py localization_service.py tests`.
- [ ] Run `.venv/bin/python -m pip check` and `.venv/bin/python -m compileall` with no broken requirements.
- [ ] Run `git diff --check` and inspect the final diff for safety invariants, secret leakage, scope expansion, stale docs, and accidental changes to `docs/superpowers/**`.
- [ ] Perform a local Streamlit smoke test without Publish/Reset writes: card placement, Danger zone, primary/reference UI, two generation paths, inactive/active uploader, Preview gating, and safe errors.
- [ ] Return a concise Russian report listing Phase changes, closed P0/P1 risks, added tests, exact verification commands/results, total test count, locale list, and unresolved issues only.
